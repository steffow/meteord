...
if [ -d /bundle ]; then
  cd /bundle
  ls -l /bundle/*
  tar xzf *.tar.gz --no-same-owner
  rm -rf /bundle/bundle/programs/server/npm/node_modules/meteor/npm-bcrypt
  ...
  rm -rf /tmp/bundle/programs/server/npm/node_modules/meteor/npm-bcrypt
  npm install
  cd /tmp/bundle/
...
if [[ $REBUILD_NPM_MODULES ]]; then
  if [ -f /opt/meteord/rebuild_npm_modules.sh ]; then
    ...
    echo "=> Use steffow/meteord-node4:bin-build for binary bulding."
    exit 1
  fi
fi
